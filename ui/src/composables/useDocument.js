export const useDocument = () => {
  const cpf = (documentNumber) => ({
    cleanDocumentNumber: documentNumber.replace(/\D/g, ''),
    masked() {
      return this.isValid()
        ? this.cleanDocumentNumber.replace(/^(\d{3})(\d{3})(\d{3})(\d{2})/, '$1.$2.$3-$4')
        : null
    },
    isValid() {
      const cpf = this.cleanDocumentNumber
      if (cpf.length !== 11) return false
      if (checkDigits()) return false

      let sum = 0
      let rest = 0

      for (let i = 1; i <= 9; i++) sum = sum + Number.parseInt(cpf.substring(i - 1, i)) * (11 - i)

      rest = (sum * 10) % 11

      if (rest === 10 || rest === 11) rest = 0
      if (rest !== Number.parseInt(cpf.substring(9, 10))) return false

      sum = 0
      for (let i = 1; i <= 10; i++) sum = sum + Number.parseInt(cpf.substring(i - 1, i)) * (12 - i)

      rest = (sum * 10) % 11

      if (rest === 10 || rest === 11) rest = 0
      if (rest !== Number.parseInt(cpf.substring(10, 11))) return false

      return true

      function checkDigits() {
        const invalidSequence = [
          '00000000000',
          '11111111111',
          '22222222222',
          '33333333333',
          '44444444444',
          '55555555555',
          '66666666666',
          '77777777777',
          '88888888888',
          '99999999999',
        ]

        return invalidSequence.includes(cpf)
      }
    },
  })

  const cnpj = (documentNumber) => ({
    cleanDocumentNumber: documentNumber.replace(/\D/g, ''),
    masked() {
      return this.isValid()
        ? this.cleanDocumentNumber.replace(/^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, '$1.$2.$3/$4-$5')
        : null
    },
    isValid() {
      const cnpj = this.cleanDocumentNumber
      if (!cnpj) return true
      if (cnpj.length !== 14) return false
      if (checkDigits()) return false

      let size = cnpj.length - 2
      const digits = cnpj.substring(size)
      let numbers = cnpj.substring(0, size)
      let sum = 0
      let pos = size - 7

      for (let i = size; i >= 1; i--) {
        sum += numbers.charAt(size - i) * pos--
        if (pos < 2) pos = 9
      }

      let result = sum % 11 < 2 ? 0 : 11 - (sum % 11)

      if (result !== Number.parseInt(digits.charAt(0))) return false

      size = size + 1

      numbers = cnpj.substring(0, size)
      sum = 0
      pos = size - 7

      for (let i = size; i >= 1; i--) {
        sum += numbers.charAt(size - i) * pos--
        if (pos < 2) pos = 9
      }

      result = sum % 11 < 2 ? 0 : 11 - (sum % 11)

      if (result !== Number.parseInt(digits.charAt(1))) return false

      return true

      function checkDigits() {
        const invalidSequence = [
          '00000000000000',
          '11111111111111',
          '22222222222222',
          '33333333333333',
          '44444444444444',
          '55555555555555',
          '66666666666666',
          '77777777777777',
          '88888888888888',
          '99999999999999',
        ]

        return invalidSequence.includes(cnpj)
      }
    },
  })

  return { cpf, cnpj }
}
